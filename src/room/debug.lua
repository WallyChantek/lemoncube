...
function level:load(prevRoom)
    ...
    -- Initialize objects.
    self.NEWPLAYER = Player:new(100, 200)
...
function level:update()
    if gamepad:isBeingHeld("up") then
        self.player:moveY(-self.player.MOVE_SPEED)
    elseif gamepad:isBeingHeld("down") then
        self.player:moveY(self.player.MOVE_SPEED)
    end
    
    if gamepad:isBeingHeld("left") then
        self.player:moveX(-self.player.MOVE_SPEED)
    elseif gamepad:isBeingHeld("right") then
        self.player:moveX(self.player.MOVE_SPEED)
    end

    if gamepad:wasPressed("fire3") then self.player:flipHorizontally() end
    if gamepad:wasPressed("fire5") then self.player:flipVertically() end
    if gamepad:isBeingHeld("fire1") then self.player:rotate(-1) end
    if gamepad:isBeingHeld("fire6") then self.player:rotate(1) end
    if gamepad:isBeingHeld("fire4") then self.player:scale(0.1, 0.1) end
    if gamepad:isBeingHeld("fire2") then
        self.player:scale(-0.1, -0.1)
        self.player:setScale(
            math.max(self.player:getHorizontalScale(), 0),
            math.max(self.player:getHorizontalScale(), 0))
    end
    
    -- if gamepad:isBeingHeld("fire4") then self.player:scaleHorizontally(0.1) end
    -- if gamepad:isBeingHeld("fire2") then self.player:scaleVertically(0.1) end
